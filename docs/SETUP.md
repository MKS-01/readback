...
- `.venv/` — ~2–3 GB.
- `~/.cache/huggingface/hub/` — ~6.5 GB (CSM weights) + ~5.5 GB per MLX model
  (default `Qwen3.5-9B-4bit`). That one model covers summaries AND image/book
  OCR, so there is no separate vision download.
