...
export function App({ handle, prefs, onQuit }: Props) {
  ...
  const persist = (voice: string, mode: "full" | "summary", model: string) =>
    savePrefs({ voice, mode, model, speed: player.getRate() });
  ...
  // /model — one picker for the one model. It drives Summary mode AND image /
  // book OCR, so there is no second (vision) list to choose from.
  const handlePickModel = (arg: string | undefined) => {
    if (!arg) {
      fetchModels()
