...
1. Put the clip in `src/voice/` as mono 24 kHz 16-bit wav — re-encode anything with
   `ffmpeg -i <in> -ac 1 -ar 24000 -sample_fmt s16 src/voice/<name>.wav`
   (renaming an .m4a to .wav does NOT work; the bytes stay AAC).
   `src/voice/*.wav` is gitignored.
2. Get the **exact transcript** (CSM conditions on the audio+text pair; a
   ...
   .venv/bin/python -c "import mlx_whisper; print(mlx_whisper.transcribe('src/voice/CLIP.wav', path_or_hf_repo='mlx-community/whisper-small-mlx')['text'].strip())"
...
Scaffolded in `src/finetune/` (see `src/finetune/README.md`). Flow:
`src/finetune/transcribe.py` → `csm-mlx finetune convert src/finetune/data src/finetune/dataset.json`
→ `csm-mlx finetune lora sft … -o src/finetune/runs/v1` (M5-tuned: `--batch-size 1
--gradient-accumulation-steps 8 --gradient-ckpt`). Then set
`tts.csm.lora_path: "src/finetune/runs/v1"` + `temperature: 0.8`. The engine loads the
